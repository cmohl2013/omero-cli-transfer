...
class TransferControl(GraphControl):
    def _configure(self, parser):
        ...
        pack.add_argument(
            "--arc",
            help="Pack into a an existing ARC repository compliant with"
            "ISA standards"
            "or create a new ARC repository if not existing so far",
            action="store_true",
        )
        pack.add_argument(
            "--simple",
        ...
        unpack.add_argument(
            "--merge",
            help="Use existing entities if possible",
            action="store_true",
        )
        unpack.add_argument(
            "--figure",
    ...
    def __pack(self, args):
        ...
        if isinstance(args.object, Image):
            ...
            return
        export_types = (args.rocrate, args.barchive, args.simple, args.arc)
        if sum(1 for ct in export_types if ct) > 1:
            raise ValueError(
                "Only one special export type (RO-Crate, Bioimage"
                " Archive, human-readable, ARC) can be specified at "
                "once"
            )
        self.metadata = []
