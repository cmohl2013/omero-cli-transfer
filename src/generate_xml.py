...
def create_line(shape: LineI) -> Line:
    ...
    if shape.getMarkerStart() is not None:
        val = shape.getMarkerStart().val
        args["marker_start"] = val if val != "None" else None
    if shape.getMarkerEnd() is not None:
        val = shape.getMarkerEnd().val
        args["marker_end"] = val if val != "None" else None
...
def create_filepath_annotations(
...
) -> Tuple[List[CommentAnnotation], List[AnnotationRef]]:
    ns = id
    anns = []
    anrefs = []
    fp_type = ns.split(":")[0]
    ...
    if fp_type == "Image":
        ...
        if len(fpaths) > 1:
            ...
            uid = (-1) * uuid4().int
            an = CommentAnnotation(id=uid, namespace=ns, value=str(path))
            anns.append(an)
            ...
            if fpaths:
                ...
                uid = (-1) * uuid4().int
                an = CommentAnnotation(id=uid, namespace=ns, value=str(f))
                anns.append(an)
                anref = AnnotationRef(id=an.id)
                anrefs.append(anref)
            else:
        ...
        uid = (-1) * uuid4().int
        an = CommentAnnotation(id=uid, namespace=ns, value=f)
        anns.append(an)
        anref = AnnotationRef(id=an.id)
        anrefs.append(anref)
    elif fp_type == "Plate":
        uid = (-1) * uuid4().int
        an = CommentAnnotation(id=uid, namespace=ns, value=plate_path)
        anns.append(an)
        anref = AnnotationRef(id=an.id)
        anrefs.append(anref)
    return anns, anrefs
...
def create_provenance_metadata(
    ...
    kv, ref = create_kv_and_ref(id=id, namespace=ns, value=Map(ms=mmap))
    return kv, ref
...
def create_objects(folder, filelist):
    ...
    for target in targets:
        ...
        imgs, pls, anns = parse_showinf(
            res, counter_imgs, counter_pls, target, folder
        )
        images.extend(imgs)
...
def parse_files_import(text, folder):
    lines = text.split("\n")
    targets = [
        line for line in lines if not line.startswith("#") and len(line) > 0
    ]
    clean_targets = []
    for target in targets:
...
def parse_showinf(text, counter_imgs, counter_plates, target, folder):
    ...
    for image in ome.images:
        ...
        img_id += 1
        uid = (-1) * uuid4().int
        an = CommentAnnotation(id=uid, namespace=img_id_str, value=target)
    ...
    for plate in ome.plates:
        ...
        pl_id += 1
        uid = (-1) * uuid4().int
        an = CommentAnnotation(id=uid, namespace=pl_id_str, value=target)
        annotations.append(an)
        anref = AnnotationRef(id=an.id)
...
def populate_roi(
    ...
    if not shapes:
        return None
    roi, roi_ref = create_roi_and_ref(
        id=id, name=name, description=desc, union=shapes
    )
...
def populate_image(
    ...
    img, img_ref = create_image_and_ref(
        id=id, name=name, description=desc, pixels=pix
    )
    ...
    if fset:
        for fs_image in fset.copyImages():
            ...
            if fs_img_id not in [i.id for i in ome.images]:
                populate_image(
                    fs_image, ome, conn, hostname, metadata, simple, fset
                )
    return img_ref
...
def populate_well(
    ...
    for index in range(obj.countWellSample()):
        ...
        ws_img_ref = populate_image(
            ws_img, ome, conn, hostname, metadata, simple=False
        )
...
def add_annotation(
    ...
    if ann.OMERO_TYPE == TagAnnotationI:
        ...
        comm, ref = create_comm_and_ref(
            id=ann.getId(), value=ann.getTextValue()
        )
...
def populate_xml_folder(
    folder: str, filelist: bool, conn: BlitzGateway, session: str
) -> Tuple[OME, dict]:
    ome = OME()
    ...
    for k, v in ids.items():
        if v.startswith("file_annotations") or v.endswith("mock_folder"):
            ...
            if top_level == "Project":
                ...
                if proj_name:
                    if (
                        proj_name + "/" + dataset_name + "/" + image_name
                    ) not in files:
                        files.append(
                            proj_name + "/" + dataset_name + "/" + image_name
                        )
...
def get_annotation_vals(cols: List[str], img: Image, ome: OME) -> List[str]:
    ...
    for annref in img.annotation_refs:
        a = next(
            filter(lambda x: x.id == annref.id, ome.structured_annotations)
        )
        anns.append(a)
    vals = []
    commented = False
